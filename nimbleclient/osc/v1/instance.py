...
class CreateInstance(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        networks = list()
        for network in parsed_args.add_networks:
            net_info = {'uuid': network.get('network-id')}
            port_type = network.get('port-type')
            if port_type:
                net_info['port_type'] = port_type
            networks.append(net_info)

        data = bc_client.instance.create(
            name=parsed_args.name,
            image_uuid=parsed_args.image,
            instance_type_uuid=parsed_args.instance_type,
