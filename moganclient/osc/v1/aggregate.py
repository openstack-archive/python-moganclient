...
class UpdateAggregate(command.Command):
    """Update an existing baremetal node aggregate"""

    def get_parser(self, prog_name):
        parser = super(UpdateAggregate, self).get_parser(prog_name)
        parser.add_argument(
            'aggregate',
            metavar='<aggregate>',
            help=_("Aggregate(s) to delete (name or UUID)")
        ...
        parser.add_argument(
            ...
            metavar="<key=value>",
            action=parseractions.KeyValueAction,
            help=_("Metadata to add to this node aggregate "
                   "(repeat option to set multiple metadatas, admin only)")
        )
        parser.add_argument(
            '--unset-metadata',
            metavar='<metadata_key>',
            action='append',
            help=_('Metadata key to unset from this node aggregate '
                   '(admin only)')
        )
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        for key, value in (parsed_args.set_metadata or {}).items():
            updates.append({"op": "add",
                            "path": "/metadata/%s" % key,
                            "value": value})
        for key in parsed_args.unset_metadata or []:
