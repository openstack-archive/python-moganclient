...
class SetFlavor(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            action=parseractions.KeyValueAction,
            help=_("Resources to add to this flavor "
                   "(repeat option to set multiple resources, admin only)")
        )
        parser.add_argument(
            "--resource-traits",
            metavar="<key=value>",
    ...
    def take_action(self, parsed_args):
        ...
        for key, value in (parsed_args.resources or {}).items():
            updates.append({"op": "add",
                            "path": "/resources/%s" % key,
                            "value": value})
        for key, value in (parsed_args.resource_traits or {}).items():
            updates.append({"op": "add",
